...
```json
{
  "primaryTaskId": "MARK-001",
  "parallelTracks": ["MARK-002", "MARK-003"],
  "dependencyGraph": {
    "MARK-001": [],
    "MARK-002": [],
    "MARK-003": ["MARK-002"]
...
```bash
node scripts/sprint-board.mjs update --id MARK-001 --state "In Progress"
node scripts/sprint-board.mjs journal --task MARK-001 --summary "what changed"
```
...
4. When marking a task `Failed`, record a pitfall entry alongside the state update:
   ```bash
   node scripts/sprint-board.mjs update --id MARK-001 --state "Failed" \
     --failure-type <gate|acceptance|review> --attempted "..." --hypothesis "..."
   node scripts/sprint-board.mjs pitfall --task MARK-001 \
     --failure-type <gate|acceptance|review> --attempted "..." --hypothesis "..." \
     [--missing-context "..."]
   ```
